...
occurrence_summary = get_bin_occurrence_summary(sample_bin, target_labels)
kable(occurrence_summary)
```
...
```{r , echo=FALSE}
joined_table = left_join(occurrence_summary, wm_records, by = c("intended_worms_taxon" = "intended_worms_taxon"))
kable(joined_table)
```
...
```{r , echo=FALSE}
# intended column headers for event table
# datasetName 	eventID 	eventDate 	decimalLongitude 	decimalLatitude 	countryCode 	geodeticDatum 	minimumDepthInMeters 	maximumDepthInMeters

# intended column headers for occurrence table
# eventID 	occurrenceID 	basisOfRecord 	identifiedBy identificationVerificationStatus 	identificationReferences 	associatedMedia 	verbatimIdentification 	scientificName 	scientificNameID 	taxonRank kingdom 	occurrenceStatus 	organismQuantity organismQuantityType institutionCode
# fill values per column
# basisOfRecord MachineObservation
# identifiedBy do not fill (leave blank for non-validated imaging dataset)
# identificationVerificationStatus PredictedByMachine
# identificationReferences Machine learning model (here is where we will recommend DOI for trained model); Software to run the machine learning model (cite version); Software to interpret autoclass scores (cite this notebook version)
# organismQuantityType counts per milliliter (is there a controlled vocabulary term for the counting unit is colony, chain, or single cell?)
